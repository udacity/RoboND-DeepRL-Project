...
#define LSTM_SIZE 32
#define NUMBER_OF_ACTIONS DOF * 2
...
bool ArmPlugin::updateAgent()
{
  ...
  const int actionDelta = (action % 2)==0 ? 1 : -1;
	...
	float velocity = vel[actions/2] + actionDelta * actionVelDelta;
	...
	float joint = ref[action/2] + actionDelta * actionVelDelta; 
